...
for i=1:m,   %first parent
	for j=1:m,		%second parent
		parent1_alles = genotypesToAlleles(i,:);
		parent2_alles = genotypesToAlleles(j,:);

		%possible_alles = [[parent1_alles(1), parent2_alles(1)], [parent1_alles(1), parent2_alles(2)], [parent1_alles(2), parent2_alles(1)], [parent1_alles(2), parent1_alles(2)]];
		possible_alles_ids = zeros(1,4);
		idx = 1;
		for a=1:2,
			for b=1:2,
				possible_alles_ids(idx) = allelesToGenotypes(parent1_alles(a), parent2_alles(b));
				idx = idx+1;
			end;
		end;

		for id = 1:4,
			index = AssignmentToIndex([possible_alles_ids(id) i j], [genotypeFactor.card]);
